...
def configuration(parent_package='', top_path=None):
    ...
    #config.add_extension(name='libscalar1d16', sources=['scalar1d_16.f90'], extra_f90_compile_args=['-Wall', '-Wextra', '-pedantic', '-Werror', '-march=native', '-O0', '-g', '-fbounds-check', '-std=f95', '-fopenmp'], extra_link_args=['-fopenmp'])
    config.add_extension(name='libscalar1d16', sources=['scalar1d_16.f90'], extra_f90_compile_args=['-Wall', '-Wextra', '-pedantic', '-Werror', '-march=native', '-O3', '-std=f95', '-fopenmp'], extra_link_args=['-fopenmp'])
